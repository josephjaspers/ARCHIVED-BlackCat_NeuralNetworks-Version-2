...
//Controls the type of mathematics back-end utilized by Tensors
//#define GPU_Tensor
//#define CPU_Tensor
...
public:
	...
	template<typename number_type> static void dot(number_type* store, unsigned store_LD, const number_type* m1, unsigned m1_row, unsigned m1_col, unsigned m1_inc,
	 ...
	 template<typename number_type> static void cross_correlation_error(unsigned cor_mv, number_type* s, const  unsigned* store_ld,  const unsigned* store_ranks, unsigned store_order,
			 	 	 	 	 	 	 	 								const number_type* filter,const  unsigned * f_ld, const  unsigned* f_ranks, unsigned f_order,
			 	 	 	 	 	 	 	 	 	 	 	 	 	 	    const number_type* signal, const unsigned * s_ld, const unsigned* s_ranks, unsigned s_order) {
		CPU_MATHEMATICS<number_type>::cc_error(cor_mv, s, store_ld, store_ranks, store_order, filter, f_ld, f_ranks, f_order, signal, s_ld, s_ranks, s_order);
	}
